...
import { ArrowLeft, Ban, Check, Download, Image as ImageIcon, KeyRound, Loader2, MessageCircle, Monitor, RotateCcw, Save, Shield, ShieldCheck, Smartphone, Users, } from "lucide-react";
import { LogoWordmark } from "@/components/logo";
import { useAuth } from "@/lib/auth-context";
import { listUsers, sendPasswordReset, grantProAccess, setUserStatus, } from "@/lib/admin.functions";
import { getAppSettings, updateAppSettings, listWhatsappChangeLog, exportWhatsappChangeLogCsv } from "@/lib/settings.functions";
import { getCitationCostStats } from "@/lib/citations.functions";
import { HelpTip } from "@/components/help-tip";
function HeroPlaceholder({ tone }) {
    const label = tone === "error"
        ? "Não foi possível carregar essa imagem."
        : tone === "warn"
            ? "URL inválida — use https://..."
            : "Sem banner configurado.";
    const hint = tone === "error"
        ? "Verifique o link ou tente outra URL."
        : tone === "warn"
            ? "Cole uma URL http(s) pública."
            : "Cole uma URL acima para pré-visualizar.";
    const ring = tone === "error"
        ? "ring-destructive/40"
        : tone === "warn"
            ? "ring-warn/40"
            : "ring-border";
    return (<div className={`grid h-full w-full place-items-center bg-gradient-to-br from-primary/10 via-background to-accent/10 ring-1 ${ring}`}>
...
function MasterPanel() {
    ...
    const [heroPreviewDevice, setHeroPreviewDevice] = useState("desktop");
    const [heroBusy, setHeroBusy] = useState(false);
    const [logoUrlInput, setLogoUrlInput] = useState("");
    const [logoBusy, setLogoBusy] = useState(false);
    const [logoPreviewStatus, setLogoPreviewStatus] = useState("idle");
    ...
    const toggleCitations = async (next) => {
    ...
    useEffect(() => {
        ...
        if (!/^https?:\/\/|^\/__l5e\//.test(url)) {
            setLogoPreviewStatus("invalid");
            return;
        }
        ...
        img.onload = () => { if (!cancelled)
            setLogoPreviewStatus("ok"); };
        img.onerror = () => { if (!cancelled)
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        img.onload = () => { if (!cancelled)
            setHeroPreviewStatus("ok"); };
        img.onerror = () => { if (!cancelled)
            setHeroPreviewStatus("error"); };
        img.src = url;
        return () => { cancelled = true; };
    }, [heroImageUrl]);
    const performRevert = async (entry) => {
    ...
    return (<div className="min-h-screen px-4 py-6 sm:px-6">
      ...
      {notice && (<div className="mx-auto mt-4 max-w-7xl rounded-xl border border-primary/40 bg-primary/10 px-4 py-2 text-sm text-primary">
          {notice}
        </div>)}
      {loadErr && (<div className="mx-auto mt-4 max-w-7xl rounded-xl border border-destructive/40 bg-destructive/10 px-4 py-2 text-sm text-destructive">
          {loadErr}
        </div>)}
      ...
      <section className="glass-panel mx-auto mt-6 max-w-7xl rounded-2xl p-5">
        ...
        <form onSubmit={saveSettings} className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="block">
            ...
            <input type="tel" inputMode="tel" placeholder="5511999998888" value={supportWa} onChange={(e) => setSupportWa(normalizeWa(e.target.value))} onBlur={(e) => setSupportWa(normalizeWa(e.target.value))} onPaste={(e) => {
            e.preventDefault();
            const txt = e.clipboardData.getData("text");
            setSupportWa(normalizeWa(txt));
        }} maxLength={16} aria-invalid={!waValid} aria-describedby="wa-hint" className={`mt-1 w-full rounded-xl bg-glass px-4 py-2.5 text-sm tabular-nums outline-none ring-1 focus:ring-2 ${waValid ? "ring-border focus:ring-primary/70" : "ring-destructive/60 focus:ring-destructive"}`}/>
            <p id="wa-hint" className={`mt-1 text-[10px] ${waValid ? "text-muted-foreground" : "text-destructive"}`}>
              {waHint}
            </p>
          </label>
          <label className="block">
            <span className="text-[11px] font-semibold uppercase text-muted-foreground">Mensagem pré-preenchida</span>
            <input type="text" placeholder="Olá! Preciso de ajuda com o Busca Mágica." value={supportMsg} onChange={(e) => setSupportMsg(e.target.value)} maxLength={280} className="mt-1 w-full rounded-xl bg-glass px-4 py-2.5 text-sm outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
          </label>
          <label className="block sm:col-span-2">
            <span className="text-[11px] font-semibold uppercase text-muted-foreground inline-flex items-center gap-1">
              ...
              <HelpTip title="Motivo da alteração" text="Fica registrado no histórico — ajuda a lembrar por que uma configuração foi trocada, útil se precisar reverter depois."/>
            </span>
            <input type="text" placeholder="Ex.: troca de plantonista, número antigo saiu, etc." value={reason} onChange={(e) => setReason(e.target.value)} maxLength={500} className="mt-1 w-full rounded-xl bg-glass px-4 py-2.5 text-sm outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
          </label>
          <div className="sm:col-span-2 flex justify-end">
            <button type="submit" disabled={settingsBusy || !waValid} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground hover:brightness-110 disabled:opacity-60">
        ...
        {settingsError && (<div className="mt-3 rounded-lg border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive">
            {settingsError}
          </div>)}
        {waValid && waDigits && (<p className="mt-3 text-[11px] text-muted-foreground">
            Preview:{" "}
            <a href={`https://wa.me/${waDigits}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
              wa.me/{waDigits}
            </a>
          </p>)}
      </section>
      ...
      <section className="glass-panel mx-auto mt-6 max-w-7xl rounded-2xl p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button type="button" onClick={downloadWaCsv} disabled={waLogExporting} className="inline-flex items-center gap-2 rounded-xl bg-glass px-3 py-2 text-xs font-semibold ring-1 ring-border hover:bg-primary/10 disabled:opacity-60">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_160px_160px_auto_auto]">
          <input type="text" placeholder="Autor (e-mail contém...)" value={waLogAuthor} onChange={(e) => setWaLogAuthor(e.target.value)} className="rounded-xl bg-glass px-3 py-2 text-xs outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
          <input type="date" value={waLogFrom} onChange={(e) => setWaLogFrom(e.target.value)} className="rounded-xl bg-glass px-3 py-2 text-xs outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
          <input type="date" value={waLogTo} onChange={(e) => setWaLogTo(e.target.value)} className="rounded-xl bg-glass px-3 py-2 text-xs outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
          <button type="button" onClick={applyWaFilters} disabled={waLogBusy} className="rounded-xl bg-primary px-3 py-2 text-xs font-bold text-primary-foreground hover:brightness-110 disabled:opacity-60">
            Filtrar
          </button>
          <button type="button" onClick={clearWaFilters} disabled={waLogBusy} className="rounded-xl bg-glass px-3 py-2 text-xs font-semibold ring-1 ring-border hover:bg-primary/10 disabled:opacity-60">
            Limpar
          </button>
        </div>

        <div className="mt-4 overflow-x-auto rounded-xl ring-1 ring-border">
          {waLog.length === 0 ? (<div className="px-4 py-6 text-center text-xs text-muted-foreground">
              ...
              <tbody>
                ...
                const canRevert = (e.old_whatsapp ?? "") !== (supportWa.replace(/\D/g, "") || "") ||
                    (e.old_message ?? "") !== (supportMsg ?? "");
                return (<tr key={e.id} className="border-t border-border/50 align-top">
                      ...
                      <td className="px-3 py-2 text-right">
                        <button type="button" onClick={() => revertWaEntry(e)} disabled={settingsBusy || !canRevert} title={canRevert ? "Reverter para o estado anterior a esta alteração" : "Estado atual já corresponde ao 'antes' deste registro"} className="inline-flex items-center gap-1 rounded-lg bg-glass px-2 py-1 text-[11px] font-semibold ring-1 ring-border hover:bg-primary/10 disabled:opacity-40">
        ...
        {waLogTotal > waLogPageSize && (<div className="mt-3 flex items-center justify-between text-[11px] text-muted-foreground">
            <span>
              Página {waLogPage} de {Math.max(1, Math.ceil(waLogTotal / waLogPageSize))} · {waLogTotal} registros
            </span>
            <div className="flex gap-2">
              <button type="button" onClick={() => changeWaPage(Math.max(1, waLogPage - 1))} disabled={waLogPage <= 1 || waLogBusy} className="rounded-lg bg-glass px-2 py-1 ring-1 ring-border hover:bg-primary/10 disabled:opacity-40">
                Anterior
              </button>
              <button type="button" onClick={() => changeWaPage(waLogPage + 1)} disabled={waLogPage >= Math.ceil(waLogTotal / waLogPageSize) || waLogBusy} className="rounded-lg bg-glass px-2 py-1 ring-1 ring-border hover:bg-primary/10 disabled:opacity-40">
                Próxima
              </button>
            </div>
          </div>)}
      ...
      <section className="glass-panel mx-auto mt-6 max-w-7xl rounded-2xl p-5">
        ...
        <form onSubmit={saveHero} className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
          <input type="url" placeholder="https://..." value={heroImageUrl} onChange={(e) => setHeroImageUrl(e.target.value)} maxLength={2048} className="w-full rounded-xl bg-glass px-4 py-2.5 text-sm outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
          <button type="submit" disabled={heroBusy} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground hover:brightness-110 disabled:opacity-60">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <label className="rounded-xl bg-glass/60 p-3 ring-1 ring-border">
            ...
            <input type="range" min={120} max={720} step={10} value={heroHeightDesktop} onChange={(e) => setHeroHeightDesktop(Number(e.target.value))} className="w-full accent-primary"/>
          </label>
          <label className="rounded-xl bg-glass/60 p-3 ring-1 ring-border">
            <div className="mb-1 flex items-center justify-between text-[11px] uppercase tracking-wider text-muted-foreground">
            ...
            <input type="range" min={100} max={480} step={10} value={heroHeightMobile} onChange={(e) => setHeroHeightMobile(Number(e.target.value))} className="w-full accent-primary"/>
          </label>
          <div className="rounded-xl bg-glass/60 p-3 ring-1 ring-border">
            ...
            <div className="grid grid-cols-2 gap-1 rounded-lg bg-muted p-1 text-xs font-bold">
              <button type="button" onClick={() => setHeroFit("cover")} className={`rounded-md px-2 py-1.5 transition ${heroFit === "cover" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                Cover
              </button>
              <button type="button" onClick={() => setHeroFit("contain")} className={`rounded-md px-2 py-1.5 transition ${heroFit === "contain" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                Contain
              </button>
            </div>
        ...
        <div className="mt-4">
          <div className="mb-2 flex items-center gap-2 text-[11px] uppercase tracking-wider text-muted-foreground">
            ...
            {heroPreviewStatus === "loading" && (<span className="inline-flex items-center gap-1 rounded-full bg-glass px-2 py-0.5 ring-1 ring-border">
                <Loader2 className="h-3 w-3 animate-spin"/> carregando
              </span>)}
            {heroPreviewStatus === "ok" && (<span className="rounded-full bg-primary/15 px-2 py-0.5 text-primary ring-1 ring-primary/40">ok</span>)}
            {heroPreviewStatus === "invalid" && (<span className="rounded-full bg-warn/15 px-2 py-0.5 text-warn ring-1 ring-warn/40">URL inválida</span>)}
            {heroPreviewStatus === "error" && (<span className="rounded-full bg-destructive/15 px-2 py-0.5 text-destructive ring-1 ring-destructive/40">falha ao carregar</span>)}
          </div>

          {/* Toggle Desktop / Mobile */}
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="inline-flex rounded-lg bg-muted p-1 text-xs font-bold">
              <button type="button" onClick={() => setHeroPreviewDevice("desktop")} aria-pressed={heroPreviewDevice === "desktop"} className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 transition ${heroPreviewDevice === "desktop" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                <Monitor className="h-3.5 w-3.5"/> Desktop
              </button>
              <button type="button" onClick={() => setHeroPreviewDevice("mobile")} aria-pressed={heroPreviewDevice === "mobile"} className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 transition ${heroPreviewDevice === "mobile" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
          ...
          <div className="overflow-x-auto rounded-2xl bg-glass/40 p-3 ring-1 ring-border">
            <div className={`relative mx-auto overflow-hidden rounded-xl bg-glass ring-1 ring-border ${heroPreviewDevice === "mobile" ? "w-[390px] max-w-full" : "w-full min-w-[640px]"}`} style={{
            height: heroPreviewDevice === "desktop" ? heroHeightDesktop : heroHeightMobile,
        }}>
              {heroImageUrl && heroPreviewStatus === "ok" ? (<img src={heroImageUrl} alt={`Prévia ${heroPreviewDevice}`} className="block h-full w-full" style={{ objectFit: heroFit, objectPosition: "center" }}/>) : heroPreviewStatus === "loading" ? (<div className="grid h-full place-items-center text-xs text-muted-foreground">
                  <Loader2 className="h-5 w-5 animate-spin text-primary"/>
                </div>) : (<HeroPlaceholder tone={heroPreviewStatus === "error" ? "error" : heroPreviewStatus === "invalid" ? "warn" : "empty"}/>)}
            </div>
          </div>
        </div>
      ...
      <section className="glass-panel mx-auto mt-6 max-w-7xl rounded-2xl p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold inline-flex items-center gap-1.5">
                ...
                <HelpTip title="Kill switch de citações web" text="Desligado por padrão de propósito, para você validar o custo real antes de liberar para os usuários. Master e admin conseguem testar mesmo com ele desligado."/>
              </h2>
          ...
          <button type="button" onClick={() => toggleCitations(!citationsEnabled)} disabled={settingsBusy} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold uppercase tracking-wider ring-1 transition ${citationsEnabled
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-[220px_1fr]">
          <label className="block">
            <span className="text-[11px] font-semibold uppercase text-muted-foreground inline-flex items-center gap-1">
              ...
              <HelpTip title="Limite diário por usuário" text="Quantidade máxima de buscas de citação que cada usuário pode fazer por dia — protege contra custo alto se muita gente usar ao mesmo tempo."/>
            </span>
            <input type="number" min={0} max={1000} value={citationsLimit} onChange={(e) => setCitationsLimit(Math.max(0, Math.min(1000, Number(e.target.value) || 0)))} onBlur={() => writeSettings({ data: { citations_daily_limit: citationsLimit } }).catch(() => { })} className="mt-1 w-full rounded-xl bg-glass px-4 py-2.5 text-sm tabular-nums outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
          </label>
      ...
      <section className="glass-panel mx-auto mt-6 max-w-7xl rounded-2xl p-5">
        ...
        <form onSubmit={saveLogo} className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
          <div className="relative">
            <input type="url" placeholder="https://sua-logo.com/imagem.png" value={logoUrlInput} onChange={(e) => setLogoUrlInput(e.target.value)} maxLength={2048} className="w-full rounded-xl bg-glass px-4 py-2.5 text-sm outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
          ...
          <button type="submit" disabled={logoBusy} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground hover:brightness-110 disabled:opacity-60">
            ...
            <div className="flex items-center gap-6 rounded-xl bg-muted/30 p-4 ring-1 ring-border">
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="grid h-16 w-16 place-items-center rounded-lg bg-background ring-1 ring-border">
                  {logoPreviewStatus === "ok" ? (<img src={logoUrlInput} alt="Preview" className="h-10 w-10 object-contain"/>) : (<ImageIcon className="h-6 w-6 text-muted-foreground/30"/>)}
                </div>
              </div>
              <div className="flex flex-col items-center gap-2">
      ...
      <div className="glass-panel mx-auto mt-6 max-w-7xl overflow-x-auto rounded-2xl">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            ...
            return (<tr key={u.id} className="border-t border-border">
                  <td className="px-4 py-3">
                    <div className="font-semibold">{u.full_name || u.email.split("@")[0]}</div>
                    <div className="text-xs text-muted-foreground">{u.email}</div>
                    {u.roles.includes("master") && (<span className="mt-1 inline-block rounded-full bg-warn/20 px-2 py-0.5 text-[10px] font-bold text-warn">MASTER</span>)}
                    {u.roles.includes("admin") && !u.roles.includes("master") && (<span className="mt-1 inline-block rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-bold text-primary">ADMIN</span>)}
                  </td>
                  <td className="px-4 py-3">
                    <PlanCell u={u}/>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex flex-wrap justify-end gap-1.5">
                      ...
                      <button onClick={() => toggleStatus(u)} disabled={busy === u.id || isSelf} title={isSelf ? "Não é possível bloquear a si mesmo" : ""} className="flex items-center gap-1 rounded-full bg-glass px-3 py-1 text-xs font-semibold ring-1 ring-border hover:bg-white/5 disabled:opacity-40">
                        {u.status === "active" ? <><Ban className="h-3 w-3"/> Bloquear</> : <><Check className="h-3 w-3"/> Ativar</>}
                      </button>
                      <button onClick={() => resetPassword(u)} disabled={busy === u.id} className="flex items-center gap-1 rounded-full bg-glass px-3 py-1 text-xs font-semibold ring-1 ring-border hover:bg-white/5 disabled:opacity-50">
...
function CostTile({ label, calls, cents, sub }) {
    const brl = cents == null ? "—" : `R$ ${(cents / 100).toFixed(2)}`;
    return (<div className="rounded-xl bg-glass p-3 ring-1 ring-border">
...
// ============================================================================
import { listAiProviderKeys, upsertAiProviderKey, deleteAiProviderKey, testAiProviderKey, testAllAiProviderKeys, saveAndTestProvider, getAiSelection, setAiSelection, PROVIDERS, PROVIDER_LABEL, PROVIDER_MODELS, } from "@/lib/ai-keys.functions";
...
function AiKeysPanel() {
    ...
    const [selection, setSelection] = useState({ mode: "auto", manual_key_id: null });
    const [busy, setBusy] = useState(null);
    const [err, setErr] = useState(null);
    const [form, setForm] = useState({ provider: "openai", label: "", secret_name: "", model: PROVIDER_MODELS.openai.default, priority: 100 });
    ...
    async function runWizard(e) {
        ...
        if (key.length < 4) {
            setWizErr("Cole a chave da API antes de salvar.");
            return;
        }
        setWizBusy(true);
        try {
    ...
    async function add(e) {
        e.preventDefault();
        if (!form.label || !form.secret_name)
            return;
        setBusy("new");
        try {
    ...
    async function changeMode(mode, manualId) {
        setBusy("mode");
        try {
    ...
    return (<section className="mx-auto mt-6 max-w-7xl px-4 sm:px-6">
      <div className="glass-panel rounded-2xl p-4 sm:p-6">
        <header className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <h2 className="text-base font-extrabold text-foreground inline-flex items-center gap-1.5">
            ...
            <HelpTip title="Failover de IA" text="O app tenta as chaves em ordem de prioridade. Se uma falha ou está fora, cai automaticamente na próxima — assim o chat de suporte e a busca de citações não param."/>
          </h2>
        ...
        <div className="mb-4 grid gap-3 rounded-xl bg-glass p-3 ring-1 ring-border sm:grid-cols-2">
          <button type="button" onClick={() => changeMode("auto")} className={`flex items-start gap-2 rounded-lg p-3 text-left ring-1 transition ${selection.mode === "auto"
          ...
          <button type="button" onClick={() => changeMode("manual", selection.manual_key_id ?? rows[0]?.id ?? null)} className={`flex items-start gap-2 rounded-lg p-3 text-left ring-1 transition ${selection.mode === "manual"
            ? "bg-primary/10 ring-primary/40"
            : "bg-background/40 ring-border hover:bg-white/5"}`}>
        ...
        <form onSubmit={runWizard} className="mb-4 rounded-xl bg-primary/5 p-4 ring-1 ring-primary/30">
          ...
          <div className="grid gap-2 sm:grid-cols-[180px_1fr_240px_auto]">
            <div>
              ...
              <select value={wiz.provider} onChange={(e) => {
            ...
            setWiz({ ...wiz, provider: next, model: PROVIDER_MODELS[next]?.default ?? "" });
                ...
                {Object.keys(PROVIDER_LABEL).map((p) => (<option key={p} className="bg-background text-foreground" value={p}>{PROVIDER_LABEL[p]}</option>))}
              </select>
            </div>
            <div>
              <label className="mb-1 block text-[10px] font-bold uppercase text-muted-foreground">2 · Chave da API</label>
              <div className="flex items-center gap-1">
                <input type={wiz.show ? "text" : "password"} autoComplete="off" spellCheck={false} placeholder={wiz.provider === "nvidia" ? "nvapi-..." : wiz.provider === "openai" ? "sk-..." : "cole a chave aqui"} value={wiz.api_key} onChange={(e) => setWiz({ ...wiz, api_key: e.target.value })} className="w-full rounded-md bg-background px-2 py-2 text-sm font-mono ring-1 ring-border"/>
                <button type="button" onClick={() => setWiz({ ...wiz, show: !wiz.show })} className="rounded-md bg-glass p-2 ring-1 ring-border hover:bg-white/5" title={wiz.show ? "Ocultar" : "Mostrar"}>
            ...
            <div>
              ...
              <select value={wiz.model || (PROVIDER_MODELS[wiz.provider]?.default ?? "")} onChange={(e) => setWiz({ ...wiz, model: e.target.value })} className="w-full rounded-md bg-background px-2 py-2 text-xs font-mono ring-1 ring-border [color-scheme:dark]">
                {(PROVIDER_MODELS[wiz.provider]?.options ?? []).map((m) => (<option key={m} className="bg-background text-foreground" value={m}>{m}</option>))}
              </select>
            </div>
            <div className="flex items-end">
              <button disabled={wizBusy} className="inline-flex h-[38px] items-center gap-1.5 rounded-md bg-primary px-4 text-xs font-extrabold text-primary-foreground shadow-lg shadow-primary/30 hover:brightness-110 disabled:opacity-50">
                <PlayCircle className="h-4 w-4"/>
                {wizBusy ? "Testando…" : "Salvar e ativar"}
              </button>
            </div>
          </div>
          ...
          {wizErr && (<div className="mt-3 flex items-start gap-2 rounded-md bg-red-500/10 p-2 text-xs text-red-300 ring-1 ring-red-500/40">
              <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0"/>
              <span className="break-words">{wizErr}</span>
            </div>)}

          {wizResult && (<div className={`mt-3 rounded-md p-3 text-xs ring-1 ${wizResult.status === "active"
              ...
              <div className="mb-1 flex items-center gap-2 text-sm font-bold">
                {wizResult.status === "active" ? (<><CheckCircle2 className="h-4 w-4"/> Conexão OK</>) : wizResult.status === "rate_limited" ? (<><AlertTriangle className="h-4 w-4"/> Chave válida, mas com limite atingido</>) : (<><AlertTriangle className="h-4 w-4"/> Falhou — veja o motivo abaixo</>)}
              </div>
        ...
        <form onSubmit={add} className="mt-3 grid gap-2 sm:grid-cols-[160px_1fr_1fr_1fr_90px_auto]">
          ...
          <select value={form.provider} onChange={(e) => {
            ...
            {PROVIDERS.map((p) => (<option key={p} className="bg-background text-foreground" value={p}>{PROVIDER_LABEL[p]}</option>))}
          </select>
          <input placeholder="Rótulo (ex: OpenAI principal)" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} className="rounded-md bg-background px-2 py-1.5 text-sm ring-1 ring-border"/>
          <input placeholder="Nome do secret (ex: NVIDIA_API_KEY)" value={form.secret_name} onChange={(e) => setForm({ ...form, secret_name: e.target.value.toUpperCase() })} className="rounded-md bg-background px-2 py-1.5 text-sm font-mono ring-1 ring-border"/>
          <div className="flex items-center gap-1">
            <input list={`models-${form.provider}`} placeholder="Modelo (ex: nvidia/llama-3.1-nemotron-70b-instruct)" value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} className="w-full rounded-md bg-background px-2 py-1.5 text-xs font-mono ring-1 ring-border"/>
            <datalist id={`models-${form.provider}`}>
              {(PROVIDER_MODELS[form.provider]?.options ?? []).map((m) => (<option key={m} value={m}/>))}
            </datalist>
            <HelpTip title="Modelo do provedor" text="Escolha um modelo compatível com o provedor (ex.: Nemotron-70b, Llama-Nemotron para NVIDIA). O teste passa a chamar esse modelo específico e o erro mostra exatamente o que falhou (autenticação, modelo indisponível, limite ou rede). Deixe em branco para usar o padrão do provedor."/>
          </div>
          <div className="flex items-center gap-1">
            <input type="number" min={1} max={999} value={form.priority} onChange={(e) => setForm({ ...form, priority: Number(e.target.value) })} title="Prioridade de failover — menor número é tentado primeiro (padrão: 100)" placeholder="Prioridade" className="w-full rounded-md bg-background px-2 py-1.5 text-sm tabular-nums ring-1 ring-border"/>
            <HelpTip title="Prioridade de failover" text="Menor número é tentado primeiro. Use 10 para a chave principal, 20/30 para reservas. Se todas empatarem em 100, a ordem fica aleatória."/>
          </div>
          <button disabled={busy === "new"} className="inline-flex items-center gap-1 rounded-md bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground hover:brightness-110 disabled:opacity-50">
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <button type="button" onClick={async () => { setBusy("all"); try {
        await testAll();
        await reload();
    }
    finally {
        setBusy(null);
    } }} disabled={busy === "all" || rows.length === 0} className="inline-flex items-center gap-1 rounded-full bg-glass px-3 py-1 text-[11px] font-bold ring-1 ring-border hover:bg-white/5 disabled:opacity-50">
        ...
        <div className="overflow-x-auto rounded-xl ring-1 ring-border">
          <table className="w-full text-xs">
            ...
            <tbody>
              ...
              {!loading && rows.length === 0 && (<tr><td colSpan={8} className="px-3 py-8 text-center text-muted-foreground">
            ...
            return (<tr key={r.id} className={`border-t border-border ${isManualPick ? "bg-primary/5" : ""}`}>
                  <td className="px-2 py-2 text-center">
                    <input type="radio" name="ai-manual" checked={isManualPick} disabled={selection.mode !== "manual" || busy === "mode"} onChange={() => changeMode("manual", r.id)} aria-label={`Usar ${r.label}`} className="h-3.5 w-3.5 accent-[oklch(var(--primary))]"/>
                  </td>
                  ...
                  <td className="px-3 py-2 font-mono text-[11px]">
                    {r.secret_name}{" "}
                    {!r.secret_present && (<span className="ml-1 rounded bg-red-500/15 px-1 py-0.5 text-[9px] font-bold text-red-300 ring-1 ring-red-400/40">
                        secret vazio
                      </span>)}
                  </td>
                  ...
                  <td className="px-3 py-2">
                    ...
                    {r.last_error && (<div className="mt-0.5 max-w-[220px] truncate text-[10px] text-red-300/80" title={r.last_error}>{r.last_error}</div>)}
                  </td>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex justify-end gap-1">
                      <button onClick={async () => { setBusy(r.id); try {
                await test({ data: { id: r.id } });
                await reload();
            }
            finally {
                setBusy(null);
            } }} disabled={busy === r.id} className="inline-flex items-center gap-1 rounded-full bg-glass px-2.5 py-1 text-[11px] font-semibold ring-1 ring-border hover:bg-white/5 disabled:opacity-50">
                        {busy === r.id ? <Loader2 className="h-3 w-3 animate-spin"/> : <PlayCircle className="h-3 w-3"/>} Testar
                      </button>
                      <button onClick={async () => {
                    if (!confirm(`Remover a chave “${r.label}”?`))
                        return;
                    setBusy(r.id);
                    try {
                ...
                }} disabled={busy === r.id} className="inline-flex items-center gap-1 rounded-full bg-glass px-2.5 py-1 text-[11px] font-semibold text-red-300 ring-1 ring-red-400/30 hover:bg-red-500/10 disabled:opacity-50">
...
function PlanCell({ u }) {
    const mode = u.pro_access_mode ?? "none";
    const until = u.pro_valid_until ?? null;
    const remaining = u.pro_searches_remaining ?? null;
    ...
    return (<div className="flex flex-col gap-1">
      ...
      {isPro && mode === "date" && until && (<span className="text-[10px] text-muted-foreground">até {new Date(until).toLocaleDateString("pt-BR")}</span>)}
      {isPro && mode === "searches" && remaining != null && (<span className="text-[10px] text-muted-foreground">{remaining} buscas restantes</span>)}
    </div>);
}
function GrantMenu({ u, busy, onGrant }) {
    ...
    const isPro = u.plan === "pro";
    if (!open) {
        return (<div className="inline-flex gap-1.5">
        <button onClick={() => setOpen(true)} disabled={busy} className="rounded-full bg-primary/15 px-3 py-1 text-xs font-semibold text-primary ring-1 ring-primary/40 hover:bg-primary/25 disabled:opacity-50">
          {isPro ? "Ajustar Pro" : "Conceder Pro"}
        </button>
        {isPro && (<button onClick={() => onGrant({ userId: u.id, mode: "free" })} disabled={busy} className="rounded-full bg-glass px-3 py-1 text-xs font-semibold ring-1 ring-border hover:bg-white/5 disabled:opacity-50">
            Voltar Free
          </button>)}
      </div>);
    }
    return (<div className="glass-panel flex flex-wrap items-center gap-1.5 rounded-xl p-2 text-xs">
      <select value={mode} onChange={(e) => setMode(e.target.value)} className="rounded bg-glass px-2 py-1 ring-1 ring-border [color-scheme:dark]">
      ...
      {mode === "date" ? (<input type="date" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} className="rounded bg-glass px-2 py-1 ring-1 ring-border"/>) : (<input type="number" min={1} value={searches} onChange={(e) => setSearches(Number(e.target.value))} className="w-20 rounded bg-glass px-2 py-1 ring-1 ring-border"/>)}
      <button onClick={() => {
            onGrant(mode === "date"
                ? { userId: u.id, mode: "date", valid_until: new Date(validUntil).toISOString() }
                : { userId: u.id, mode: "searches", searches_granted: searches });
            setOpen(false);
        }} disabled={busy} className="rounded-full bg-primary px-3 py-1 font-bold text-primary-foreground hover:brightness-110 disabled:opacity-50">
        Aplicar
      </button>
      <button onClick={() => setOpen(false)} className="rounded-full bg-glass px-3 py-1 ring-1 ring-border hover:bg-white/5">
        Cancelar
      </button>
    </div>);
}
