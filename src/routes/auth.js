...
function AuthPage() {
    ...
    const handleGoogle = async () => {
        ...
        try {
            ...
            if (safeRedirect && typeof window !== "undefined") {
                try {
                    sessionStorage.setItem("bm.postLoginRedirect", safeRedirect);
                }
                catch { /* quota */ }
            }
        ...
        if (!loading && user) {
            ...
            if (typeof window !== "undefined") {
                try {
                    ...
                    if (stashed)
                        sessionStorage.removeItem("bm.postLoginRedirect");
                }
                catch { /* ignore */ }
            }
    ...
    return (<div className="grid min-h-screen place-items-center px-4 py-10">
      <div className="w-full max-w-md">
        ...
        <div className="glass-panel rounded-2xl p-6 sm:p-8">
          ...
          {mode !== "forgot" && (<>
              <button type="button" onClick={handleGoogle} disabled={googleLoading || submitting} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-white py-2.5 text-sm font-bold text-gray-900 ring-1 ring-border transition hover:brightness-95 disabled:opacity-60">
                {googleLoading ? (<Loader2 className="h-4 w-4 animate-spin"/>) : (<svg className="h-4 w-4" viewBox="0 0 48 48" aria-hidden="true">
          ...
          <form onSubmit={handle} className={mode === "forgot" ? "mt-6 space-y-3" : "space-y-3"}>
            ...
            {mode === "signup" && (<label className="block">
                ...
                <input type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)} className="mt-1 w-full rounded-xl bg-glass px-4 py-2.5 text-sm outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/70"/>
              </label>)}
            <label className="block">
              ...
              <div className="mt-1 flex items-center gap-2 rounded-xl bg-glass px-4 py-2.5 ring-1 ring-border focus-within:ring-2 focus-within:ring-primary/70">
                ...
                <input type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-transparent text-sm outline-none"/>
              </div>
            </label>
            {mode !== "forgot" && (<label className="block">
                ...
                <div className="mt-1 flex items-center gap-2 rounded-xl bg-glass px-4 py-2.5 ring-1 ring-border focus-within:ring-2 focus-within:ring-primary/70">
                  ...
                  <input type="password" required minLength={8} autoComplete={mode === "login" ? "current-password" : "new-password"} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-transparent text-sm outline-none"/>
                </div>
                {mode === "signup" && (<span className="mt-1 block text-[10px] text-muted-foreground">Mínimo 8 caracteres.</span>)}
              </label>)}
            {mode === "login" && (<div className="flex items-center justify-between text-xs">
                <label className="flex items-center gap-2 text-muted-foreground">
                  <input type="checkbox" defaultChecked className="h-3.5 w-3.5 rounded border-border bg-glass accent-primary"/>
                  Lembrar de mim
                </label>
            ...
            {error && (<div className="flex items-start gap-2 rounded-lg border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive">
                <AlertCircle className="mt-0.5 h-4 w-4 shrink-0"/>
                <span>{error}</span>
              </div>)}
            {info && (<div className="rounded-lg border border-primary/40 bg-primary/10 px-3 py-2 text-xs text-primary">
                {info}
              </div>)}
            <button type="submit" disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-2.5 text-sm font-bold text-primary-foreground transition hover:brightness-110 hover:neon-primary disabled:opacity-60">
          ...
          <div className="mt-5 flex flex-col gap-1.5 text-center text-xs text-muted-foreground">
            ...
            {mode === "signup" && (<button onClick={() => setMode("login")} className="hover:text-primary">
                Já tem conta? <span className="text-primary">Entrar</span>
              </button>)}
            {mode === "forgot" && (<button onClick={() => setMode("login")} className="hover:text-primary">
                Voltar para login
              </button>)}
          </div>
        </div>
