...
import { exportToCsv, getContacted, getFavorites, getHistory, toggleContacted, toggleFavorite, } from "@/lib/storage";
...
function filterSaved(items, terms) {
    if (terms.length === 0)
        return items;
    return items.filter((l) => matchesAny(`${l.name} ${l.address} ${l.phone ?? ""} ${l.website ?? ""}`, terms));
}
function filterHistory(items, terms) {
    if (terms.length === 0)
        return items;
    return items.filter((h) => matchesAny(`${h.query} ${h.region}`, terms));
}
function SavedList({ title, icon, items, onRemove, emptyText, filename, terms, }) {
    return (<section className="glass-panel rounded-2xl p-4">
      <header className="mb-3 flex items-center justify-between">
        ...
        <button onClick={() => exportToCsv(items.map((l) => ({
            nome: l.name,
            endereco: l.address,
            telefone: l.phone ?? "",
            ...
            salvo_em: l.saved_at,
        })), filename)} disabled={items.length === 0} className="inline-flex h-9 items-center gap-1.5 rounded-full bg-primary px-4 text-xs font-bold text-primary-foreground shadow-elevated hover:brightness-110 disabled:opacity-40">
      ...
      {items.length === 0 ? (<p className="text-sm text-muted-foreground">
          ...
          {items.map((l) => (<li key={l.place_id} className="flex items-center justify-between gap-2 rounded-xl bg-glass p-3 ring-1 ring-border">
              <div className="min-w-0">
                ...
                <div className="mt-1 flex gap-3 text-[11px] text-muted-foreground">
                  ...
                  {l.website && (<a href={l.website} target="_blank" rel="noreferrer" className="text-primary hover:underline">
                      site
                    </a>)}
                  {l.google_maps_uri && (<a href={l.google_maps_uri} target="_blank" rel="noreferrer" className="text-primary hover:underline">
                      maps
                    </a>)}
                </div>
              </div>
              <button onClick={() => onRemove(l)} className="rounded-md bg-glass p-1.5 text-muted-foreground ring-1 ring-border hover:text-destructive" aria-label={`Remover ${l.name}`}>
      ...
      <main className="mx-auto grid max-w-6xl gap-4 px-4 pb-16 sm:px-6 lg:grid-cols-2">
        <div className="lg:col-span-2">
          <label className="flex items-center gap-2 rounded-xl bg-glass px-4 py-3 ring-1 ring-border focus-within:ring-2 focus-within:ring-primary/70">
            ...
            <input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Filtrar por nome, endereço, telefone ou site (separe termos por espaço ou vírgula)…" aria-label="Filtrar leads salvos" className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"/>
            {filter && (<button onClick={() => setFilter("")} aria-label="Limpar filtro" className="rounded p-1 text-muted-foreground hover:text-foreground">
                <X className="h-4 w-4"/>
              </button>)}
          </label>
          {terms.length > 0 && (<p className="mt-1.5 text-[11px] text-muted-foreground">
              {favsView.length + doneView.length} lead(s) e {historyView.length} busca(s) encontrada(s).
            </p>)}
        </div>

        <SavedList title="Favoritos" icon={<BookmarkCheck className="h-4 w-4"/>} items={favsView} filename="favoritos.csv" emptyText="Ainda sem favoritos. Clique no ícone de marcador nos cards da busca." onRemove={(l) => setFavs(toggleFavorite(l))} terms={terms}/>
        <SavedList title="Contatados" icon={<CheckCircle2 className="h-4 w-4"/>} items={doneView} filename="contatados.csv" emptyText="Ainda sem contatados. Marque os leads que você já abordou." onRemove={(l) => setDone(toggleContacted(l))} terms={terms}/>
        ...
        <section className="glass-panel rounded-2xl p-4 lg:col-span-2">
          ...
          {historyView.length === 0 ? (<p className="text-sm text-muted-foreground">
              {terms.length > 0 ? "Nenhuma busca corresponde ao filtro." : "Nenhuma busca ainda."}
            </p>) : (<ul className="divide-y divide-border">
              {historyView.map((h) => (<li key={h.id} className="flex flex-wrap items-center justify-between gap-2 py-2 text-sm">
                  <div>
